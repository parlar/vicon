...
The pipeline processes each sample through the following stages: (1) adapter trimming, (2) host read removal, (3) automatic reference selection (enabled by default), (4) reference preparation and segment labelling, (5) optional de novo reference seeding, (6) convergence-based iterative mapping with optional reference elongation, (7) de novo assembly with multiple assemblers, (8) iVar per-contig consensus calling, (9) assembly-guided polishing, (10) multi-sequence alignment of all pipeline stages, (11) phylogenetic backbone tree construction and individual consensus placement, (12) homoplasy analysis, (13) best consensus selection and hybrid consensus construction, (14) consensus validation by re-placement on the backbone tree, (15) read support analysis at disputed sites, and (16) per-sample and cross-sample HTML reporting. Stages 6–15 are run in parallel for two analysis lines: one with PCR duplicate removal ("dedup") and one without ("nodedup"), enabling direct comparison of deduplication effects on consensus accuracy (Ebbert et al. 2016).

### Adapter trimming

Raw paired-end reads are preprocessed with fastp (Chen et al. 2018), which performs automatic detection and removal of sequencing adapters (`--detect_adapter_for_pe`) and quality trimming. fastp generates per-sample HTML and JSON quality control reports summarising read quality distributions, adapter content, and filtering statistics. This step ensures that downstream mapping and assembly are not confounded by residual adapter sequences or low-quality bases.

### Host read removal

Adapter-trimmed reads are filtered against host reference genomes using Bowtie2 (Langmead and Salzberg 2012) in `--very-sensitive` mode. For hantavirus samples from bank voles (*Myodes glareolus*), reads are sequentially mapped against a human genome index and a bank vole genome index. Unmapped read pairs from both filtering steps are retained for downstream analysis. This sequential approach ensures removal of both human contamination (from sample handling) and host reads, a critical preprocessing step for metagenomic and clinical viral samples (Matranga et al. 2014).

### Automatic reference selection

ViCon automatically selects the best reference for each genomic segment by default. This is particularly important given the substantial genetic diversity among hantavirus lineages, even within a single geographic region (Lundkvist et al. 2008; Johansson et al. 2024). Host-filtered reads are mapped with Minimap2 (Li 2018) against user-provided reference FASTA files for each segment (L, M, S), configurable via the `genbank_refs` option or a reference database directory. For each candidate reference, ViCon computes the fraction of positions covered at >2x depth (completeness) and the total number of mapped reads. References are ranked by completeness (primary) and mapped read count (secondary), and the best reference per segment is selected. The per-segment references are combined into a single multi-segment FASTA for downstream analysis.

### Reference preparation

Reference contigs are matched to user-configured segment names (default: L, M, S for hantaviruses) by contig header. The segment list is configurable, supporting any number of segments for different viral families (e.g. a single segment for non-segmented viruses, or eight segments for influenza). Matched contigs are renamed to a standardised `{segment}_{sample}` format and the reference is indexed for the selected aligner (BWA-MEM2 or strobealign) and samtools.
...
- **Trinity** (Grabherr et al. 2011): RNA-Seq assembler designed for transcript reconstruction, run via Apptainer container
...
|-----------|---------|-------------|
| `segments` | [L, M, S] | Genomic segments to process |
...
Samples are auto-detected from paired-end FASTQ files in the input directory, or a subset can be specified explicitly. Dependencies are managed by Pixi with packages from bioconda (Grüning et al. 2018) and conda-forge, ensuring reproducibility through lockfile-based dependency resolution. Trinity is run via Apptainer (Kurtzer et al. 2017), enabling rootless container execution on HPC systems without requiring Docker.
